...
p. Under Extensions tab you will find a subpage called _Autogrowing Textareas_ where you can set all sort of settings and create a so called _textarea callbacks_ which will basically hook the auto-growing function to the specific textarea.

p. Defining (and hooking) the textarea is done by choosing the backend page where the textarea exists and the XHTML ID (for example body, excerpt etc.) which is defined for the textarea. The auto-grown textarea can either be plugin's or Textpattern's core textarea. Any valid textarea with ID (located under valid @event@ -get callback) can be made to auto-grow.

h2. Changelog

h3. Version 0.7 - upcoming

* Improved: Makes sure values saved via the editor indeed are integers.
* Improved: Escapes element name passed to CSS styles.
* Changed: Now uses Textpattern's core's @script_js()@ and @escape_js()@ functions.
* Removed: Documentation link.
* Now requires Textpattern version 4.4.0 or newer.
