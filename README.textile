...
p. rah_expanding allows admin-side themes and plugins to set rules for the expanding textareas. Themes have the ability to limit the fields dimensions or disable the plugin on a field basis. All this is doable with some basic, general CSS rules.
...
p. Automatic expanding can be disabled on a field basis. Any textarea element that is armed with a class @rah_expanding_disable@ will not be affected by the plugin.
...
p. The plugin works for textareas present in the initial page load. If the DOM modified after that and a new textarea is added, the the plugin's resizer needs to be attached to those elements. The plugin allows using a jQuery interface. It has a method named @rah_expanding_resize()@.

bc. $('textarea').rah_expanding_resize();

p. The above would attach the resizing magic to any new textarea present on the page.

h3. Triggering resize

p. A textarea's resizing can be invoked manually incase the field's contents are changed programmatically on-the-fly. These type of automated changes are not otherwise tracked without the plugin being specifically informed about the change. rah_expanding has a custom event named as @rah_expanding_resize@. You can access it using jQuery's "trigger":http://api.jquery.com/trigger/ method.
