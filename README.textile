...
p. Rah_expanding brings dynamically expanding textareas to "Textpattern CMS'":http://textpattern.com admin-side interfaces near you. With the plugin textarea elements will grow with a user's input. While the user types, the field's height will increase to fit the content.
...
p. Browsers that outright miss required features will gracefully fallback to normal textareas with visible scrollbars.
...
* Cleaned up the code.
* Changed: Text flowing workaround for Chrome. Tries to improve the text wrapping of the initial textarea contents.
