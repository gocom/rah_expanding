...
p. "Project page":http://rahforum.biz/plugins/rah_autogrowing_textarea | "Twitter":http://twitter.com/gocom | "GitHub":https://github.com/gocom/rah_expanding | "Support forum":http://forum.textpattern.com/viewtopic.php?id=27646 | "Donate":http://rahforum.biz/donate/rah_expanding

p. Automatically expanding textareas for "Textpattern CMS'":http://textpattern.com admin-side interfaces near you. With the plugin textarea elements will grow with user's input. While the user types, the field's height will increase to fit the content.

h2. Requirements

p. Rah_expanding's minimum requirements:

* Textpattern v4.4.1 and newer.
* PHP 5.2 and newer.
...
p. Web browsers that outright miss required features will gracefully fallback to a normal textarea with visible scrollbars.
...
p. Rah_expanding gives some additional tools for developers to play with. The plugin allows admin-side themes and plugins to set rules for the dynamic expanding textareas. Themes have the ability to limit the fields dimensions or disable the plugin on a field basis. All this is doable with some basic, general CSS rules.
...
p. A field's maximum height that limits the field's expanding, can be set with a "max-height":https://developer.mozilla.org/en/CSS/max-height CSS property. If a "textarea":https://developer.mozilla.org/en/HTML/Element/textarea doesn't have max-height, it expands all the way up to 99999 pixels.
...
p. By default the plugin works for textareas present in the initial page load. If the DOM is modified after that and a new textarea is added, the the plugin's expanding behavior needs to be attached to those new elements. Rah_expanding sports a jQuery plugin interface, and introduces a method named @rah_expanding()@.
